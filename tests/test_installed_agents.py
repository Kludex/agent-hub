...
async def test_installed_agents_are_pinned_updated_with_confirmation_and_removed(tmp_path: Path) -> None:
    ...
    async with httpx2.AsyncClient() as client:
        ...
        await catalog.install("agent-hub/implementation-planner", "1.0.0")
        listing = await execute_installed_agent_command(service, ("list",))
        updated = await execute_installed_agent_command(
            service, ("update", "agent-hub/implementation-planner"), "1.1.0"
        )
        current = await service.update("agent-hub/implementation-planner", "1.1.0")
        with pytest.raises(PermissionExpansionError, match="tools: write_file.*write access.*network access.*MCP"):
            await service.update("agent-hub/implementation-planner", "2.0.0")
        expanded = await execute_installed_agent_command(
            service, ("update", "agent-hub/implementation-planner"), "2.0.0", confirmed=True
        )
        removed = await execute_installed_agent_command(service, ("remove", "agent-hub/implementation-planner"))

    assert listing == "agent-hub/implementation-planner 1.0.0 - pydantic-ai, read-only"
    assert updated == "Updated agent-hub/implementation-planner from 1.0.0 to 1.1.0"
    assert current == "agent-hub/implementation-planner is already at 1.1.0"
    assert expanded == "Updated agent-hub/implementation-planner from 1.1.0 to 2.0.0"
    assert removed == "Removed agent-hub/implementation-planner 2.0.0"
...
async def test_installed_agent_commands_report_missing_duplicates_and_usage(tmp_path: Path) -> None:
    ...
    async with httpx2.AsyncClient() as client:
        ...
        with pytest.raises(InstalledAgentError, match="not found"):
            service.remove("agent-hub/implementation-planner")
        with pytest.raises(InstalledAgentError, match="Usage"):
            await execute_installed_agent_command(service, ("unknown",))

        await catalog.install("agent-hub/implementation-planner", "1.0.0")
        first = data_dir / "agents" / "agent-hub" / "implementation-planner" / "1.0.0"
...
def _registry_with_updates(tmp_path: Path) -> Path:
    ...
    for version in ("1.0.0", "1.1.0", "2.0.0"):
        ...
        if version == "2.0.0":
            ...
            manifest = manifest.replace("network_access = false", "network_access = true")
            manifest = manifest.replace(
                'tools = ["read_file", "list_files"]', 'tools = ["read_file", "list_files", "write_file"]'
            )
            manifest = manifest.replace(
