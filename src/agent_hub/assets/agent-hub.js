...
var JSONLDecoder = class {
  constructor(maxRecordBytes = 1024 * 1024) {
...
// src/client.ts
var TERMINAL_STATES = /* @__PURE__ */ new Set(["succeeded", "failed", "aborted"]);
var HubClient = class {
  ...
  async health(signal) {
    await this.httpRequest("GET", "/health", void 0, signal);
  }
...
`);
    ...
    const record2 = records.find((item) => item.id === id);
    if (!record2) {
      throw new Error("Agent Hub returned no matching JSON-RPC response");
    }
    if (record2.error) {
      throw new Error(`${record2.error.message} (${record2.error.code})`);
    }
  ...
  async waitForRun(runId, after, onEvent, signal) {
    let sequence = after;
    while (true) {
      const current = await this.rpc("run.get", { runId }, signal);
      ...
      if (consumed.gap) {
        const snapshot = await this.rpc("hub.snapshot", {}, signal);
        sequence = snapshot.latestSequence;
      }
    }
  }
  consumeEvents(runId, after, onEvent, signal) {
          ...
          response.on("data", (chunk) => {
            try {
              for (const notification of decoder.push(chunk)) {
                ...
                onEvent(event);
                if (event.runId === runId && event.type === "run.state.changed" && TERMINAL_STATES.has(event.data.state)) {
                  terminal = true;
                  response.destroy();
                }
              }
  ...
  httpRequest(method, path, body, signal) {
    return new Promise((resolve, reject) => {
      const req = request(
        {
          ...
          path,
          method,
          headers: body ? { "content-type": "application/x-ndjson", "content-length": String(body.length) } : void 0
        },
        (response) => {
          const chunks = [];
          response.on("data", (chunk) => chunks.push(chunk));
          response.on("end", () => {
            if (response.statusCode === void 0 || response.statusCode >= 400) {
...
async function showHub(ctx, client) {
  await client.ensureAvailable();
  while (true) {
    let snapshot;
    try {
      snapshot = await client.rpc("hub.snapshot");
    } catch {
      await client.ensureAvailable();
      snapshot = await client.rpc("hub.snapshot");
    }
...
async function inspectAgent(ctx, client, selected) {
  const detail = await client.rpc("agent.get", { agentId: selected.id });
  ...
  if (action === "Follow live output") {
    ...
    const snapshot = await client.rpc("hub.snapshot");
    ...
    try {
      ...
      ctx.ui.setStatus("agent-hub-follow", void 0);
    }
    const updated = await client.rpc("agent.get", { agentId: detail.agent.id });
    await ctx.ui.editor(`${detail.agent.profile} transcript`, transcript(updated));
    return;
  }
  if (action === "Inspect patch") {
    const result2 = await client.rpc("agent.patch", {
      agentId: detail.agent.id
    });
    const suffix = result2.truncated ? "\n\n[Patch truncated by Agent Hub]" : "";
    await ctx.ui.editor(`${detail.agent.profile} patch`, result2.patch + suffix);
    return;
  }
  if (action === "Return result to editor") {
    const result2 = [...detail.runs].reverse().find((run) => run.result)?.result;
    if (result2) {
      ctx.ui.setEditorText(result2);
    }
    return;
  }
  if (action === "Steer" || action === "Follow up" || action === "Prompt") {
    ...
    const method2 = action === "Steer" ? "agent.steer" : action === "Follow up" ? "agent.follow_up" : "agent.prompt";
    const key = action === "Prompt" ? "prompt" : "message";
    await client.rpc(method2, {
      agentId: detail.agent.id,
      [key]: message,
  ...
  if ((action === "Apply patch" || action === "Discard worktree") && !await ctx.ui.confirm(action, `Confirm ${action.toLowerCase()} for ${detail.agent.profile}?`)) {
    return;
  }
  const methods = {
    Abort: "agent.abort",
  ...
  if (method) {
    await client.rpc(method, { agentId: detail.agent.id, idempotencyKey: randomUUID2() });
  }
}
function availableActions(agent) {
...
function label(agent, activeRuns) {
...
function elapsedLabel(startedAt) {
  const seconds = Math.max(0, Math.floor((Date.now() - Date.parse(startedAt)) / 1e3));
...
function GetProperties(type) {
  const result2 = IsObject2(type) ? type.properties : IsTuple(type) ? TupleElementsToProperties(type.items) : {};
  return result2;
}
...
function NarrowCompareRule(left, right) {
  ...
  return guard_exports.IsEqual(result2, CompareResultLeftInside) ? left : guard_exports.IsEqual(result2, CompareResultRightInside) ? right : guard_exports.IsEqual(result2, CompareResultEqual) ? right : Never();
}
...
function ShouldEvaluate(left, right) {
  const result2 = IsUnion(left) || IsUnion(right);
  return result2;
}
...
function ExcludeUnion(left, right, result2 = []) {
  return guard_exports.ShiftLeft(left, (head, tail) => ExcludeUnion(tail, right, [...result2, ...ExcludeType(head, right)]), () => result2);
}
...
function EvaluateDependent(if_, then_, else_) {
  ...
  const result2 = EvaluateUnion([intersected, excluded]);
  return result2;
}
...
function DelimitedDecode(input, result2 = []) {
  return guard_exports.ShiftLeft(input, (left, right) => DelimitedDecode(right, [...result2, left[1]]), () => result2);
}
...
var GenericParameterList_0 = (input, result2 = []) => If(If(Const(",", input), ([_0, input2]) => If(GenericParameter(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => GenericParameterList_0(input2, [...result2, _0]), () => [result2, input]);
...
var GenericCallArgumentList_0 = (input, result2 = []) => If(If(Const(",", input), ([_0, input2]) => If(Type(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => GenericCallArgumentList_0(input2, [...result2, _0]), () => [result2, input]);
...
var PropertyList_0 = (input, result2 = []) => If(If(PropertyDelimiter(input), ([_0, input2]) => If(Property(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => PropertyList_0(input2, [...result2, _0]), () => [result2, input]);
...
var ElementList_0 = (input, result2 = []) => If(If(Const(",", input), ([_0, input2]) => If(Element(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => ElementList_0(input2, [...result2, _0]), () => [result2, input]);
...
var ParameterList_0 = (input, result2 = []) => If(If(Const(",", input), ([_0, input2]) => If(Parameter2(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => ParameterList_0(input2, [...result2, _0]), () => [result2, input]);
...
var WithPropertyList_0 = (input, result2 = []) => If(If(PropertyDelimiter(input), ([_0, input2]) => If(WithProperty(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => WithPropertyList_0(input2, [...result2, _0]), () => [result2, input]);
...
var WithElementList_0 = (input, result2 = []) => If(If(Const(",", input), ([_0, input2]) => If(WithValue(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => WithElementList_0(input2, [...result2, _0]), () => [result2, input]);
...
var InterfaceDeclarationHeritageList_0 = (input, result2 = []) => If(If(Const(",", input), ([_0, input2]) => If(Type(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => InterfaceDeclarationHeritageList_0(input2, [...result2, _0]), () => [result2, input]);
...
var ModuleDeclarationList_0 = (input, result2 = []) => If(If(ModuleDeclarationDelimiter(input), ([_0, input2]) => If(ModuleDeclaration(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => ModuleDeclarationList_0(input2, [...result2, _0]), () => [result2, input]);
...
function BroadenFilter(type, types, result2 = [], all = types) {
  return guard_exports.ShiftLeft(types, (left, right) => {
    ...
    return guard_exports.IsEqual(compare, CompareResultLeftInside) || guard_exports.IsEqual(compare, CompareResultEqual) ? all : guard_exports.IsEqual(compare, CompareResultDisjoint) ? BroadenFilter(type, right, [...result2, left], all) : BroadenFilter(type, right, result2, all);
  }, () => [...result2, type]);
}
...
function ExtractUnion(left, right, result2 = []) {
  return guard_exports.ShiftLeft(left, (head, tail) => ExtractUnion(tail, right, [...result2, ...ExtractType(head, right)]), () => result2);
}
...
function FromTuple(types) {
  ...
  const result2 = FromType8(object2);
  return result2;
}
...
// src/task-tool.ts
function registerTaskTool(pi, client) {
  pi.registerTool({
    name: "task",
    label: "Task",
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const snapshot = await client.rpc("hub.snapshot", {}, signal);
      const spawned = await client.rpc(
        "agent.spawn",
        {
...
function normalizeUsage(raw) {
  ...
  const input = number2(tokens.input ?? raw.input ?? raw.inputTokens);
  const output = number2(tokens.output ?? raw.output ?? raw.outputTokens);
  const cacheRead = number2(tokens.cacheRead ?? raw.cacheRead ?? raw.cacheReadTokens);
  const cacheWrite = number2(tokens.cacheWrite ?? raw.cacheWrite ?? raw.cacheWriteTokens);
  const totalCost = typeof raw.cost === "number" ? number2(raw.cost) : number2(cost.total);
  return {
    input,
    output,
    cacheRead,
    cacheWrite,
    totalTokens: number2(tokens.total ?? raw.totalTokens) || input + output + cacheRead + cacheWrite,
    cost: {
      input: number2(cost.input),
      output: number2(cost.output),
      cacheRead: number2(cost.cacheRead),
      cacheWrite: number2(cost.cacheWrite),
      total: totalCost
    }
  };
}
...
// src/index.ts
function registerAgentHub(pi) {
  const socketPath = process.env.AGENT_HUB_SOCKET ?? join(homedir(), ".agent-hub", "run", "agent-hub.sock");
