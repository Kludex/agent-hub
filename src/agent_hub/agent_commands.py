...
async def spawn(manager: AgentManager, params: dict[str, Any]) -> dict[str, Any]:
    ...
    model = manager._model_override(profile, params.get("model"))
    request = StartAgentRequest(agent_id, profile, cwd, manager.config.data_dir / "sessions" / agent_id, model, access)
    try:
        manager.runtimes[runtime_name].validate(request)
    except RuntimeFailure as exc:
        raise RPCError(-32602, redact_text(str(exc))) from exc
    ...
    manager._start_run(run, profile, model, isolated)
