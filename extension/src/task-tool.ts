...
export function registerTaskTool(pi: ExtensionAPI, client: HubClient): void {
  pi.registerTool({
    ...
    label: "Task",
    description:
      "Delegate a task to an Agent Hub profile. Prefer profile defaults for model and runtime; use background for long work. Results are limited to 50KB.",
    ...
    parameters: Type.Object({
      ...
      background: Type.Optional(
        Type.Boolean({
          default: false,
          description:
            "Return a handle without waiting. Use for long work, not a shorter deadline.",
        }),
      ),
      model: Type.Optional(
        Type.Union([Type.String(), Type.Null()], {
          description:
            "Omit or pass null to use the profile default (preferred). Overrides must match the profile's runtime and installed providers, not the parent Pi model.",
        }),
      ),
      ...
      maxRuntimeSeconds: Type.Optional(
        Type.Number({
          minimum: 1,
          description:
            "Optional lower runtime cap in seconds, not a wait timeout. Omit to use the profile default; cannot extend its maximum. Avoid arbitrary short caps; use background for long work.",
        }),
      ),
    }),
